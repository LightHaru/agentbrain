...
function scoreOne(probe: RecallProbe, returnedIds: string[], k: number): ProbeRecallScore {
  const topK = returnedIds.slice(0, k);
  const relevant = new Set(probe.relevantIds);
  const relevantCount = relevant.size || 1;

  let hit = 0;
  for (const id of topK) if (relevant.has(id)) hit++;
  ...
  for (let i = 0; i < topK.length; i++) {
    if (relevant.has(topK[i])) { mrr = 1 / (i + 1); break; }
  }
...
export const GOLDEN_RECALL_CORPUS: GoldenEntry[] = [
