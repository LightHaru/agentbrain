...
export class SqlStorageAdapter {
  ...
  constructor(brainDir: string) {
    ...
    this.db = new BrainDatabase(dbPath);
  }
  ...
  async writeMemoryFile(type: string, memories: Memory[]): Promise<void> {
      ...
      for (const mem of memories) {
        ...
        if (mem.accessCount > 0) {
          this.db.raw(
            ...
            [mem.accessCount, mem.lastAccessed, mem.id]
          );
        }
      }
    });
  ...
  async writeFile(path: string, content: string): Promise<void> {
    ...
    if (path === 'learning/lessons.md') {
      ...
      } catch (e) { /* store raw */ this.db.setMeta(`file:${path}`, content); }
      return;
    }
    if (path === 'learning/patterns.md') {
      try {
      ...
      } catch (e) { this.db.setMeta(`file:${path}`, content); }
      return;
    }
  ...
  private buildEmotionalStateFile(): string {
    const state = this.db.getMeta('emotional_state');
    return state || '# Emotional State\nMood: neutral\nValence: 0.0\nArousal: 0.3\nIntensity: 0.2';
  }

  private buildRelationshipFile(): string {
  ...
  private buildPersonalityFile(): string {
  ...
  private buildReflectionsFile(): string {
    const stored = this.db.getMeta('file:reflection/daily.md');
    return stored || '';
  }

  private buildSkillsFile(): string {
  ...
  private buildHabitsFile(): string {
  ...
  private parseAndSaveEmotionalState(content: string): void {
    this.db.setMeta('emotional_state', content);
  }

  private parseAndSaveRelationships(content: string): void {
    ...
    for (const line of lines) {
      ...
      if (headerMatch) {
        if (current?.user_id) this.db.upsertRelationship(current);
        current = { user_id: headerMatch[2], user_name: headerMatch[1] };
      } else if (current) {
        const m = line.match(/^- Depth:\s*([\d.]+)/);
        if (m) current.depth = parseFloat(m[1]);
        const t = line.match(/^- Trust:\s*([\d.]+)/);
        if (t) current.trust_level = parseFloat(t[1]);
        ...
        if (l) current.last_interaction = l[1].trim();
      }
    }
    if (current?.user_id) this.db.upsertRelationship(current);
  }

  private parseAndSavePersonality(content: string): void {
    const lines = content.split('\n');
    for (const line of lines) {
  ...
  private parseAndSaveSkills(content: string): void {
    const lines = content.split('\n');
    let current: any = {};
    ...
    for (const line of lines) {
      ...
      if (p) current.proficiency = parseFloat(p[1]);
      const u = line.match(/^- Used:\s*(\d+)/);
      if (u) current.timesUsed = parseInt(u[1]);
      const l = line.match(/^- Last:\s*(.+)/);
      if (l) current.lastUsed = l[1].trim();
      const s = line.match(/^- Success:\s*(\d+)\/(\d+)/);
      if (s) { current.successes = parseInt(s[1]); current.failures = parseInt(s[2]) - parseInt(s[1]); }
      const c = line.match(/^- Category:\s*(.+)/);
      if (c) current.category = c[1].trim();
    }
    if (current.name) {
      this.db.upsertSkill({
  ...
  private parseAndSaveHabits(content: string): void {
    const lines = content.split('\n');
    let current: any = {};
    let idx = 0;
    ...
    for (const line of lines) {
      ...
      if (a) current.action = a[1].trim();
      const f = line.match(/^- Frequency:\s*(\d+)/);
      if (f) current.frequency = parseInt(f[1]);
      const c = line.match(/^- Confidence:\s*([\d.]+)/);
      if (c) current.confidence = parseFloat(c[1]);
      const fs = line.match(/^- First:\s*(.+)/);
      if (fs) current.firstSeen = fs[1].trim();
      const ls = line.match(/^- Last:\s*(.+)/);
      if (ls) current.lastSeen = ls[1].trim();
    }
    if (current.pattern) {
      this.db.upsertHabit({
        id: `habit-${idx}`, pattern: current.pattern,
