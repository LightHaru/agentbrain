 ...
 */
export function stability(accessCount: number, confidence: number): number {
  const base = 3;
...
export function retention(input: RetentionInput): number {
 ...
 */
export function shouldForget(input: RetentionInput, threshold = 0.18): boolean {
  // Never forget something still actively used and trusted.
  if (input.accessCount >= 5 && input.confidence >= 0.6) return false;
 ...
 */
export function decayedConfidence(input: RetentionInput): number {
