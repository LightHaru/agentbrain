...
export interface GraphNode {
  ...
  label: string;       // original display text
  kind: 'entity' | 'value';
}
...
export function isCleanFactRow(f: { subject?: string; object?: string }): boolean {
...
export class MemoryGraph {
  ...
  private out = new Map<string, GraphEdge[]>();   // adjacency: node id -> edges
  private in = new Map<string, GraphEdge[]>();     // reverse adjacency
  private built = false;
  ...
  build(): void {
    ...
    for (const f of facts) {
      ...
      if (!this.in.has(toId)) this.in.set(toId, []);
      this.in.get(toId)!.push(edge);
    }
    this.built = true;
  }

  private addNode(label: string, kind: GraphNode['kind']): string {
    const id = norm(label);
    if (!this.nodes.has(id)) this.nodes.set(id, { id, label, kind });
    return id;
  }
  ...
  stats(): { nodes: number; edges: number } {
    ...
    let edges = 0;
    for (const list of this.out.values()) edges += list.length;
    return { nodes: this.nodes.size, edges };
  }

  /** Find node ids whose label shares tokens with the query. */
  private matchNodes(query: string): string[] {
    ...
    for (const node of this.nodes.values()) {
      ...
      if (hit > 0) scored.push({ id: node.id, score: hit + node.label.length * 0.001 });
    }
   ...
   */
  recallConnected(query: string, opts: { maxHops?: number; limit?: number } = {}): ConnectedResult {
    if (!this.built) this.build();
    const maxHops = opts.maxHops ?? 2;
    const limit = opts.limit ?? 6;

    const seeds = this.matchNodes(query);
    if (seeds.length === 0) return { facts: [], nodesVisited: 0 };

    const visited = new Set<string>();
    const collected = new Map<string, { edge: GraphEdge; hops: number }>();
    let frontier = [...seeds];
    ...
    for (let hop = 1; hop <= maxHops && frontier.length > 0; hop++) {
      ...
      for (const nodeId of frontier) {
        ...
        for (const e of edges) {
          if (!collected.has(e.factId)) collected.set(e.factId, { edge: e, hops: hop });
   ...
   */
  formatForInjection(query: string, opts: { maxHops?: number; limit?: number } = {}): string {
