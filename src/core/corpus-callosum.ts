...
export interface BrainSignal {
  ...
  target: string | '*'; // specific module or broadcast
...
export class CorpusCallosum {
  ...
  private processedSignals: BrainSignal[] = [];
  private conflicts: ConflictReport[] = [];
  private listeners: Map<string, ((signal: BrainSignal) => void)[]> = new Map();
   ...
   */
  broadcast(source: string, type: BrainSignal['type'], payload: any, priority: BrainSignal['priority'] = 'normal'): string {
    return this.send({
      source,
      target: '*',
      type,
   ...
   */
  alert(source: string, alertPayload: { level: string; message: string; data?: any }): string {
    return this.send({
      source,
      target: '*',
      type: 'alert',
   ...
   */
  processQueue(): { processed: number; dropped: number } {
    const now = Date.now();
    let processed = 0;
    let dropped = 0;
   ...
   */
  heartbeat(): { processed: number; dropped: number; activeModules: number } {
   ...
   */
  private processSignal(signal: BrainSignal): void {
   ...
   */
  private notifyListeners(moduleId: string, signal: BrainSignal): void {
   ...
   */
  private resolveConflict(modules: string[], issue: string): string {
