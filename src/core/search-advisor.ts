...
interface SignalHit {
  reason: string;
  weight: number; // contribution toward "required"
}

export class SearchAdvisor {
  // Time-sensitivity: answer changes over time, so memory is likely stale.
  private timeSensitive: RegExp[] = [
  ...
  private externalFactual: RegExp[] = [
   ...
   */
  advise(message: string, opts: { hasWebSearch?: boolean } = {}): SearchAdvice {
    const text = (message || '').trim();
    const none: SearchAdvice = { urgency: 'none', reasons: [], directive: '', suggestedTools: [] };
    ...
    if (this.any(this.explicit, text)) signals.push({ reason: 'user explicitly asked to look it up / cite a source', weight: 1.0 });
    if (this.any(this.volatile, text)) signals.push({ reason: 'volatile/live fact (price, news, version, market) that memory cannot be trusted for', weight: 0.8 });
    if (this.any(this.timeSensitive, text)) signals.push({ reason: 'time-sensitive phrasing (latest/current/recent/date) — remembered data may be stale', weight: 0.6 });
    if (isQuestion && this.any(this.externalFactual, text)) signals.push({ reason: 'factual question about the external world', weight: 0.4 });
    ...
    let urgency: SearchUrgency = 'none';
    if (!internal) {
      if (score >= 0.8) urgency = 'required';
      else if (score >= 0.4) urgency = 'recommended';
    }
  ...
  private any(patterns: RegExp[], text: string): boolean {
