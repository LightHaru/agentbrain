...
export class ReasoningCortex {
   ...
   */
  async generateWhisper(context: WhisperContext): Promise<BrainWhisper> {
    ...
    try {
      ...
      const reasoningFrame = this.generateReasoningFrame(
        taskAnalysis,
        confidence,
        memories,
        thinkingMode,
      ...
      const verificationChecks = this.generateVerificationChecks(
        taskAnalysis,
        confidence,
        thinkingMode,
        playbooks
      ...
      const uncertaintySignals = this.detectUncertaintySignals(
        context.userMessage,
        taskAnalysis,
        confidence,
        memories,
  ...
  private getAdvisorModelHint(): AdvisorModelHint | undefined {
   ...
   */
  private detectTimePressure(context: WhisperContext): number | undefined {
  ...
  private generateAdaptiveSuggestions(
    analysis: TaskAnalysis,
    confidence: { overall: number; hasKnowledge: boolean },
    timePressure: number | undefined,
    tokenBudget: number,
    playbooks: ReasoningPlaybook[] = []
  ): string[] {
    const heuristic = this.HEURISTICS[analysis.type as keyof typeof this.HEURISTICS];
    if (!heuristic) return [];
  ...
  private getSuggestedApproach(
    ...
    if (trainedApproach) return trainedApproach;
    ...
    const approachObj = (heuristic as any).approach;
  ...
  private detectUncertaintySignals(
    ...
    const signals: string[] = this.unique(playbooks.flatMap((playbook) => playbook.uncertaintySignals));
    const msg = message.toLowerCase();
  ...
  private generateSourcePlan(playbooks: ReasoningPlaybook[]): string[] {
    return this.unique(this.orderPlaybooks(playbooks, false).flatMap((playbook) => playbook.sourcePlan)).slice(0, 8);
  }

  private generateAnswerContract(playbooks: ReasoningPlaybook[]): string[] {
  ...
  private hasAnyPlaybook(playbooks: ReasoningPlaybook[], ids: string[]): boolean {
  ...
  private prioritizePlaybookHints(
    hints: string[],
    playbooks: ReasoningPlaybook[],
    prioritySubstrings: string[]
  ): string[] {
    if (!this.hasPlaybook(playbooks, 'frontend-artifact-quality')) {
      return hints;
    }

    const prioritized = prioritySubstrings.flatMap((substring) =>
      hints.filter((hint) => hint.toLowerCase().includes(substring.toLowerCase()))
    );

    return this.unique([...prioritized, ...hints]);
  }

  private orderPlaybooks(playbooks: ReasoningPlaybook[], genericFirst: boolean): ReasoningPlaybook[] {
  ...
  private playbookSpecificity(playbook: ReasoningPlaybook): number {
    if (playbook.id === 'market-token-price-live') return 1;
    if (playbook.id === 'evidence-triangulation-live') return 2;
    return 0;
  }
   ...
   */
  private getEffectiveSuggestions(taskType: TaskType): string[] {
    const relevantOutcomes = this.outcomes.filter(
      o => o.taskType === taskType && o.success && o.userSatisfaction && o.userSatisfaction > 0.7
    );
   ...
   */
  private calculateTokenBudget(context: WhisperContext, analysis: TaskAnalysis): number {
    ...
    const frontendArtifactRequested =
      /(landing|frontend|website|web site|webapp|web app|html|css|ui|responsive|mobile|desktop|component|page|trang|trang web|giao dien|ung dung web)/i.test(intentText) &&
    ...
    const codeToolRequested =
      /(code|tool|command|script|test|build|plugin|api|hook|file|repo|debug|fix|bug|cong cu|lenh|ma nguon|tap tin|tep|sua bug)/i.test(intentText) &&
  ...
  private async queryRelevantMemories(
    ...
    try {
      ...
      const semanticResults = await this.hippocampus.recall(
        message,
        'semantic'
      );
      ...
      if (tokenBudget >= 150) {
        const episodicResults = await this.hippocampus.recall(
          message,
          'episodic'
        );
  ...
  private analyzeTask(message: string, context: WhisperContext): TaskAnalysis {
    ...
    const frontendArtifactIntent =
      /(landing|frontend|website|web site|webapp|web app|html|css|ui|responsive|mobile|desktop|component|page|trang|trang web|giao dien|ung dung web)/i.test(msg) &&
    ...
    const codeToolIntent =
      /(code|tool|command|script|test|build|plugin|api|hook|file|repo|debug|fix|bug|cong cu|lenh|ma nguon|tap tin|tep|sua bug)/i.test(msg) &&
  ...
  private assessConfidence(
    analysis: TaskAnalysis,
    memories: string[]
  ): { overall: number; hasKnowledge: boolean } {
    ...
    const baseFamiliarity: Record<TaskType, number> = {
      'factual-lookup': 0.8,
      'market-data': 0.65,
      'troubleshooting': 0.7,
      'planning': 0.75,
   ...
   */
  private getSuccessRate(taskType: TaskType): number {
  ...
  private generateCautions(
    ...
    return this.prioritizePlaybookHints(
      cautions,
      playbooks,
      ['generic AI template', 'Over-clean dashboards', 'verified working page']
    ).slice(0, maxCautions + (playbooks.length > 0 ? 1 : 0));
  }
  ...
  getStatistics(): {
    ...
    return {
      ...
      taskTypeBreakdown: breakdown as Record<TaskType, number>,
    };
  }
}
