...
export interface WorkspaceSnapshot {
  ...
  integrationScore: number; // how well modules are integrated
}
...
export class GlobalWorkspace {
  ...
  private currentFocus: ConsciousContent | null = null;
  private recentContents: ConsciousContent[] = [];
  private streamOfConsciousness: string[] = [];
  private attentionFilter: string[] = [];
  private consciousnessLevel: number = 0.7;
  private competitionQueue: ConsciousContent[] = [];
  private competitionsResolved: number = 0;
  ...
  submit(content: Omit<ConsciousContent, 'id' | 'timestamp'>): string {
    ...
    const fullContent: ConsciousContent = {
      ...content,
      id,
      timestamp: Date.now(),
    };
  ...
  compete(): CompetitionResult | null {
    if (this.competitionQueue.length === 0) return null;
   ...
   */
  getStream(limit: number = 10): string[] {
   ...
   */
  getRecentContents(limit: number = 5): ConsciousContent[] {
   ...
   */
  integrate(inputs: { source: string; content: string; weight: number }[]): ConsciousContent {
    ...
    const integrated: ConsciousContent = {
  ...
  getIntegrationScore(): number {
    if (this.recentContents.length === 0) return 0.5;

    // Count unique sources in recent contents
    const recentSources = new Set(
      this.recentContents.slice(-20).map(c => c.source)
    );
