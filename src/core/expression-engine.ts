...
export interface ExpressionProfile {
  ...
  kaomojiPool: string[];
  tone: string; // e.g. "ấm áp, trìu mến"
  energy: Energy;
  verbosity: Verbosity;
  /** small speech habits that flavor the voice, e.g. "hehe", "…", "!!" */
  verbalTics: string[];
  punctuation: string; // guidance on punctuation/exclamation usage
 ...
 */
const LIBRARY: Record<EmotionLabel, EmotionStyle> = {
  joy: {
    moodWord: 'vui',
    kaomoji: ['(◕‿◕✿)', '(＾▽＾)', '(≧▽≦)', 'ヽ(´▽`)/', '(*≧ω≦*)', '(๑˃ᴗ˂)ﻭ'],
...
const ENERGY_ORDER: Energy[] = ['flat', 'low', 'steady', 'lively', 'high'];

function shiftEnergy(base: Energy, steps: number): Energy {
...
export class ExpressionEngine {
  ...
  render(input: ExpressionInput): ExpressionProfile {
    ...
    if (intensity >= 0.66) verbosity = 'expansive';
    else if (intensity < 0.33) verbosity = 'terse';
    if (energy === 'flat') verbosity = 'terse';
  ...
  private buildDirective(
    ...
    if (kaomoji) parts.push(`Có thể chèn kaomoji như ${kaomoji}.`);
    if (tics.length) parts.push(`Chất giọng: "${tics.join('", "')}".`);
...
function energyVi(energy: Energy): string {
  switch (energy) {
