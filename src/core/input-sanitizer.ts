...
const INJECTION_CUT_MARKERS: RegExp[] = [
 ...
 */
export function sanitizeUserMessage(raw: string | undefined | null): SanitizeResult {
  const empty: SanitizeResult = { clean: '', strippedInjection: false, redactedSecret: false };
  if (!raw) return empty;
  ...
  if (/Sender \(untrusted metadata\)|Conversation context \(untrusted|Conversation info \(untrusted metadata\)/i.test(text)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (/^\s*#(\d+|session:)/i.test(lines[i])) lastHist = i;
    }
    if (lastHist >= 0) {
  ...
  while (changed) {
    ...
    for (const p of LEADING_PREFIXES) {
      ...
      if (next !== text) { text = next; changed = true; strippedInjection = true; }
    }
  }
 ...
 */
export function redactSecrets(raw: string | undefined | null): { clean: string; redactedSecret: boolean } {
  if (!raw) return { clean: '', redactedSecret: false };
  let text = raw;
  let redactedSecret = false;
  for (const { re, label } of SECRET_PATTERNS) {
    if (re.test(text)) { redactedSecret = true; text = text.replace(re, label); }
    re.lastIndex = 0;
  }
