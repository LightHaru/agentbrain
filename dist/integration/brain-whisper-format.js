...
function formatWhisper(whisper) {
    ...
    if (whisper.advisorModel?.enabled) {
        lines.push(`Advisor model: ${whisper.advisorModel.model} (${whisper.advisorModel.role}); critique/check only, never final answer.`);
    }
...
function inferFeedbackOutcome(message, sentiment) {
    const msg = message.toLowerCase();
    const looksLikePositiveFeedback = /\b(thanks|thank you|good job|great|perfect|excellent|nice|well done|correct|right|dung|chuan|cam on)\b/.test(msg);
    const looksLikeNegativeFeedback = /\b(wrong|incorrect|bad answer|not correct|not right|you were wrong|failed|sai|khong dung|chua dung)\b/.test(msg);
...
function getInjectionBudget(reasoningWhisper, baseTokens = 250) {
    if (!reasoningWhisper) {
        return baseTokens;
    }
...
//# sourceMappingURL=brain-whisper-format.js.map