...
class SourceVerifier {
    ...
    advise(message, opts = {}) {
        ...
        const none = { needed: false, entityKind: 'generic', candidates: [], directive: '' };
        if (text.length < 3)
            return none;
        if (INTERNAL.test(text) && !TOKEN_CUES.test(text))
            return none;
        const isToken = TOKEN_CUES.test(text);
        const isEntity = ENTITY_CUES.test(text);
        if (!isToken && !isEntity)
            return none;
        const entityKind = isToken ? 'token'
    ...
    extractCandidates(text, kind) {
        const out = new Set();
        if (kind === 'token') {
            for (const m of text.matchAll(TICKER)) {
                const t = m[0].replace('$', '');
                // Skip common non-ticker uppercase words.
                if (!/^(THE|AND|FOR|USD|USDT|API|CEO|NFT|DEX|CEX|CA)$/.test(t))
                    out.add(t);
            }
        }
    ...
    buildDirective(kind, candidates, hasTool) {
...
exports.SourceVerifier = SourceVerifier;
