...
class Thalamus {
     ...
     */
    detectIntent(msg) {
        if (/\?|hỏi|là gì|thế nào|sao|how|what|why|when/.test(msg))
            return 'question';
        if (/làm|tạo|build|code|write|viết|deploy|fix/.test(msg))
            return 'action_request';
        if (/tìm|search|research|check|xem|kiểm tra/.test(msg))
            return 'research';
        if (/nhắc|remind|alarm|cron|schedule|lịch/.test(msg))
            return 'reminder';
        if (/ok|ừ|oke|được|đồng ý|yes|confirm|chốt/.test(msg))
            return 'confirmation';
        if (/không|no|cancel|hủy|thôi|dừng|stop/.test(msg))
            return 'rejection';
        if (/cảm ơn|thanks|thank|tks/.test(msg))
            return 'gratitude';
        return 'statement';
    }
    /**
    ...
    detectUrgency(msg) {
        ...
        if (/xong.*chưa|đâu.*rồi|tới.*đâu|sao.*rồi|thế.*nào.*rồi/i.test(msg)) {
            ...
            if (/gấp|nhanh|ngay|liền|asap/i.test(msg))
                return 'high';
            return 'medium';
        }
     ...
     */
    routeToModules(classification) {
        const modules = ['hippocampus']; // always recall memory
...
exports.Thalamus = Thalamus;
//# sourceMappingURL=thalamus.js.map