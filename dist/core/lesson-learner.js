...
class LessonLearner {
    ...
    analyze(params) {
        ...
        if (!strongestSignal || maxSeverity < 0.5)
            return null;
        ...
        if (isTeasingSignal && isHighTrust && hasNicknameHabit) {
            ...
            if (maxSeverity < 0.5)
                return null; // Don't create lesson for teasing
        }
     ...
     */
    findRelevantLessons(query, limit = 3) {
     ...
     */
    addLesson(lesson) {
        const existing = this.lessons.find((l) => l.trigger === lesson.trigger && l.right === lesson.right);
        if (existing) {
            existing.occurrences++;
    ...
    // Private
    // ==========================================================================
    extractLesson(userMessage, previousResponse, signal) {
    ...
    findSimilarLesson(trigger) {
        const tokenize = (s) => s.toLowerCase().normalize('NFC')
...
exports.LessonLearner = LessonLearner;
//# sourceMappingURL=lesson-learner.js.map