...
exports.TimeAwareness = void 0;
exports.buoiOf = buoiOf;
// ── Buổi (part of day), Vietnamese ─────────────────────────────────────────
function buoiOf(hour) {
    if (hour >= 5 && hour < 11)
        return 'sáng';
    if (hour >= 11 && hour < 13)
        return 'trưa';
...
class TimeAwareness {
    ...
    parts(ms) {
        ...
        if (hour === 24)
            hour = 0; // some ICU emit 24 at midnight
        return {
            hour,
    ...
    holidayFor(y, mo, d) {
        ...
        if (tet) {
            ...
            if (diffDays === -1)
                return 'Giao thừa (30 Tết)';
            if (diffDays === 0)
                return 'Mùng 1 Tết Nguyên Đán';
            if (diffDays === 1)
                return 'Mùng 2 Tết';
            if (diffDays === 2)
                return 'Mùng 3 Tết';
            if (diffDays >= 3 && diffDays <= 6)
                return `Tết Nguyên Đán (mùng ${diffDays + 1})`;
            if (diffDays >= -7 && diffDays <= -2)
                return 'Cận Tết (sắp Tết)';
        }
        return null;
    }
    /** Current-time context for injection. */
    nowContext(nowMs = Date.now()) {
        ...
        if (holiday)
            line += ` — hôm nay là ${holiday}`;
        if (p.hour >= 0 && p.hour < 5)
            line += ' (đang khuya, giờ này Sếp thường ngủ)';
        return {
     ...
     */
    describeTimestamp(tsISO, nowMs = Date.now()) {
     ...
     */
    inferGap(prevISO, nowMs = Date.now()) {
...
exports.TimeAwareness = TimeAwareness;
//# sourceMappingURL=time-awareness.js.map