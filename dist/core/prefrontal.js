...
exports.PrefrontalCortex = void 0;
class PrefrontalCortex {
     ...
     */
    estimateComplexity(classification, message) {
        const wordCount = message.split(/\s+/).length;
        if (!classification.requiresAction)
            return 'trivial';
        if (wordCount < 10 && classification.urgency === 'low')
            return 'simple';
        if (/và|and|rồi|then|sau đó|next|also|thêm/.test(message))
            return 'moderate';
        if (/project|system|architecture|toàn bộ|refactor|migrate/.test(message))
            return 'complex';
        if (/build.*from scratch|redesign|rewrite|toàn diện/.test(message))
            return 'epic';
        return 'simple';
    }
    /**
     ...
     */
    decompose(classification, message, complexity) {
     ...
     */
    logDecision(context, decision, reasoning, confidence) {
        this.decisions.push({
            timestamp: new Date().toISOString(),
            context,
            decision,
     ...
     */
    updateWorkingMemory(content, source, relevance) {
...
`;
    ...
    parsePlan(content) {
        ...
        return {
            ...
            subTasks: [],
            status: statusMatch?.[1] || 'in_progress',
            estimatedComplexity: complexityMatch?.[1] || 'simple',
        };
    }
...
exports.PrefrontalCortex = PrefrontalCortex;
