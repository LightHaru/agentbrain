...
exports.SemanticPlaybookMatcher = void 0;
exports.allPlaybooks = allPlaybooks;
...
class SemanticPlaybookMatcher {
    ...
    async warmup(playbooks = allPlaybooks()) {
        // Force the model to load by embedding a probe.
        const probe = await this.engine.embed('warmup');
        if (!probe || this.engine.hasFailed())
            return false;
        for (const pb of playbooks) {
            if (!pb.intentAnchors || pb.intentAnchors.length === 0)
                continue;
            if (this.anchorCache.has(pb.id))
                continue;
            const embs = await this.engine.embedBatch(pb.intentAnchors);
            const valid = embs.filter((e) => e !== null);
            if (valid.length > 0)
                this.anchorCache.set(pb.id, valid);
        }
     ...
     */
    async match(message, playbooks = allPlaybooks()) {
        const regexMatches = (0, reasoning_playbooks_js_1.matchReasoningPlaybooks)(message);
        if (!this.isReady())
            return regexMatches;
        ...
        for (const pb of playbooks) {
            ...
            for (const a of anchors)
                best = Math.max(best, cosine(msgEmb, a));
            if (best >= this.threshold)
                scored.push({ pb, score: best });
        }
    ...
    async bestScore(message, playbookId) {
        if (!this.isReady())
            return 0;
        const msgEmb = await this.engine.embed(message);
...
exports.SemanticPlaybookMatcher = SemanticPlaybookMatcher;
function allPlaybooks() {
    return [...reasoning_playbooks_js_1.REASONING_PLAYBOOKS, ...(0, reasoning_playbooks_js_1.getLearnedPlaybooks)()];
}
//# sourceMappingURL=semantic-playbook-matcher.js.map