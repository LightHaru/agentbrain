...
exports.HELDOUT_PROBES = exports.DEFAULT_PROBES = void 0;
exports.scoreProbe = scoreProbe;
exports.runBenchmark = runBenchmark;
exports.scoreProbeSemantic = scoreProbeSemantic;
exports.runBenchmarkSemantic = runBenchmarkSemantic;
const reasoning_playbooks_js_1 = require("../core/reasoning-playbooks.js");
/** Default probe suite spanning core reasoning skills. */
exports.DEFAULT_PROBES = [
...
function scoreProbe(message, lessonLearner) {
    const playbooks = (0, reasoning_playbooks_js_1.matchReasoningPlaybooks)(message);
...
function runBenchmark(probes = exports.DEFAULT_PROBES, lessonLearner) {
...
async function scoreProbeSemantic(message, matcher, lessonLearner) {
...
async function runBenchmarkSemantic(matcher, probes = exports.DEFAULT_PROBES, lessonLearner) {
    const probeScores = [];
    for (const p of probes) {
        probeScores.push({ ...(await scoreProbeSemantic(p.message, matcher, lessonLearner)), id: p.id });
    }
    const total = probeScores.reduce((s, p) => s + p.score, 0) / (probeScores.length || 1);
    return { total, probeScores, timestamp: new Date().toISOString() };
}
